...
- Hijri Calendar module (`src/hijri-calendar/`) with:
  - Core APIs: `computeHijriDate`, `computeHijriMonth`, `computeHijriRange`
  - Calculation methods: `ummul_qura` (table) and `nu_falakiyah` (IRNU)
  - Adjustment system (none/memory/json/provider-sync)
  - Utilities: validators, formatters, grid builder
